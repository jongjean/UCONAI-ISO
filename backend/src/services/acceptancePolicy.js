...
export function buildFinalEngineCompletionSnapshot(input = {}) {
  ...
  const gateEvidence = [
    ...
    runtime.storageSnapshotCreated ? "migration-plan" : null,
    runtime.backupEvidenceCreated ? "backup-plan" : null,
    runtime.restoreEvidenceCreated ? "rollback-plan" : null,
    publicService.frontendOk ? "hp-preflight" : null,
    publicService.exportJobOk ? "docx-render-sample" : null,
    publicService.exportJobOk ? "figure-source-package" : null,
    ai.provider === "ollama" && ai.model ? "role-matrix" : null,
    ai.provider === "ollama" && ai.model ? "secret-check" : null,
